...
class Controller:
    ...
    def record_data(self):
        ...
        motor_idx = 4
        if self.low_cmd.motor_cmd[motor_idx].q == 0:
            return
        ...
        self.q_cmd_data.append(self.low_cmd.motor_cmd[motor_idx].q)
    ...
    def plot_data(self):
        ...
        fig.suptitle('Motor 1 State and Command Over Time')

        start = 0
        end = 500
        # 1. Position Plot
        axs[0].plot(self.time_data[start:end], self.qpos_data[start:end], label='Actual Position (q)')
        axs[0].plot(self.time_data[start:end], self.q_cmd_data[start:end], label='Command Position (q_cmd)', linestyle='--', color='red')
        axs[0].set_ylabel('Position (rad)')
        axs[0].grid(True)
        axs[0].legend()

        # 2. Velocity Plot (Actual vs Command)
        axs[1].plot(self.time_data[start:end], self.qvel_data[start:end], label='Actual Velocity (dq)', color='blue')
        axs[1].plot(self.time_data[start:end], self.dq_cmd_data[start:end], label='Command Velocity (dq_cmd)', linestyle='--', color='red')
        axs[1].set_ylabel('Velocity (rad/s)')
        axs[1].grid(True)
        axs[1].legend()

        # 3. Torque Plot (Estimated vs Command)
        axs[2].plot(self.time_data[start:end], self.qtau_data[start:end], label='Estimated Torque (tau_est)', color='green')
        axs[2].plot(self.time_data[start:end], self.tau_cmd_data[start:end], label='Command Torque (tau_cmd)', linestyle=':', color='red')
